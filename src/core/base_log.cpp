...
{	
	M_Arena* arena = ArenaAlloc(MB(32));
...
internal void LogMsg(Logger* logger, LogMsgKind kind, String8 msg)
{
	...
	if(kind == LogMsgKind_Info)
		log = Str8Format(temp.Arena, "\x1b[%dm[INFO]\x1b[0m %S", ANSI_FG_GREEN, msg);
	else if(kind == LogMsgKind_Warning)
		log = Str8Format(temp.Arena, "\x1b[%dm[WARN]\x1b[0m %S", ANSI_FG_YELLOW, msg);
	else if(kind == LogMsgKind_Error)
		log = Str8Format(temp.Arena, "\x1b[%dm[ERROR]\x1b[0m %S", ANSI_FG_B_RED, msg);
	else if(kind == LogMsgKind_Panic)
		log = Str8Format(temp.Arena, "\x1b[%dm[PANIC]\x1b[0m %S", ANSI_FG_RED, msg);		
	else Assert(0);
